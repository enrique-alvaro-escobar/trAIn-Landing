...
export const SUST_FPS = FPS;
export const SUST_DURATION = 602;
...
const SceneSwaps: React.FC = () => (
  <Scene dur={182}>
    <div style={{ display: "flex", flexDirection: "column", gap: 56 }}>
      <Header badge="03" title={"Sustituciones\ninteligentes"} />
      <div style={{ display: "flex", flexDirection: "column", gap: 50 }}>
        <SwapRow bad="¿Sin barras?" good="Goblet squat" delay={30} />
        <SwapRow bad="¿Lesión?" good="Máquina Smith" delay={48} />
        <SwapRow bad="¿Aburrido?" good="Variante similar" delay={66} />
      </div>
    </div>
  </Scene>
);
...
const SceneAlternatives: React.FC = () => (
  <Scene dur={182}>
    <div style={{ display: "flex", flexDirection: "column", gap: 46 }}>
      <div>
        <Eyebrow>Mismo objetivo</Eyebrow>
        <BigTitle>Otra vía, misma meta.</BigTitle>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 26 }}>
        <AltRow name="Triceps Pushdown" tag="Mismo grupo · sin barra" delay={26} top />
        <AltRow name="Cable Overhead Ext." tag="Tríceps · cable" delay={42} />
        <AltRow name="Fondos asistidos" tag="Tríceps · peso corporal" delay={58} />
      </div>
    </div>
  </Scene>
);
...
const SceneDone: React.FC = () => {
  ...
  return (
    <Scene dur={170}>
      <div style={{ display: "flex", flexDirection: "column", gap: 50 }}>
        <div>
          <Eyebrow>Sustituido</Eyebrow>
          <BigTitle>Sin perder el objetivo.</BigTitle>
        </div>
        <div
          style={{
            background: C.cardBg,
        ...
        <div style={{ display: "flex", gap: 18, ...useFadeUp(48, 16) }}>
          {["2 × 8", "Intensidad alta", "Sin barra"].map((t) => (
            <span
              key={t}
              style={{
                fontFamily: FONT,
...
export const SustReel: React.FC = () => (
  <LoopWrap>
    <Sequence from={0} durationInFrames={96}>
      <Hook lines={["¿Sin barra?", "¿Lesión? Da igual."]} dur={96} />
    </Sequence>
